...
export default function App() {
  ...
  const url = useMemo(() => `${API}/api/flights/airport/${airport}`, [airport]);
  ...
  useEffect(() => {
    ...
    fetch(url)
      ...
      .then(setFlights)
      .catch(() => setError("Could not load flights. Is the API running on :8080?"));
  }, [url]);
  ...
  return (
    <div style={{ maxWidth: 900, margin: "40px auto", padding: 16, fontFamily: "system-ui" }}>
      ...
      {error && <div style={{ padding: 12, background: "#ffe3e3", borderRadius: 8 }}>{error}</div>}
      ...
      <p style={{ marginTop: 18, opacity: 0.7 }}>
        Next steps: add Admin CRUD screens for Flights/Airports/Airlines/Gates + forms + validation.
      </p>
    </div>
  );
}
